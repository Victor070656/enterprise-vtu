...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (isset($get_data)) {
		...
		$iuc = $conn->real_escape_string(test_input($get_data->iuc));
	} else {
		...
		$iuc = $conn->real_escape_string(test_input($_REQUEST['iuc']));
	}
	$amountPayee = "950";
	$auth = "paid";
			...
			if ($arr === $pair) {
				...
				if ($debit < $mid_wxpi) {
					...
					if ($nu == 0) {
						...
						function n3tNabteb($conn, $quantity)
						{
							function fetchEpin($conn)
							{
						...
						$resp = json_decode(n3tNabteb($conn, $iuc), true);
						...
						if ($resp->status == 'success' && !empty($pin) or $pin !== NULL) {
							$stat = "Completed";
							// debit account
						...
						$addQuery = $conn->query("INSERT INTO transactions (network,serviceid,channel,phone,amount,charge,ref,status,date,email,newBal,metertoken,customer,qty) VALUES('$product_name <br> PIN:$tok','$variation_code','$channel','$CustomerPhone','$amount','$debit','$requestid','$stat','$dateTime','$email','$newBalc','$tok','$customNam','$iuc')");
					} else {
