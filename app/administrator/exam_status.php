...
$resp['status'] = true;
$resp['redirect'] = "exam_package.php";
echo json_encode($resp);
exit(); 
} else {
...
$fetchRecord = $conn->query("SELECT * FROM exam_package WHERE network='$network'");
$rowz = $fetchRecord->fetch_assoc();
return json_encode($rowz);
}  
function UpdateSingle($conn,$status,$network){    
$Update_single = $conn->query("UPDATE exam_package SET status='$status' WHERE  network='$network'");
