...
<body class="fixed-nav sticky-footer bg-dark" id="page-top">
 ...
 <?php include('inc/nav.php');?>
  ...
  <div class="content-wrapper">
    ...
    <div class="container-fluid">
      ...
      <?php 
		$sn = $_GET['id'];
		function tv($conn,$sn){
		 $Runqry = $conn->prepare("SELECT * FROM tv_package WHERE serial=?");
  ...
  <form action="javascript:void(0)" method="post" autocomplete = 'off' id="updatetvplan" > 
     ...
     <table class="table  margin-tp-10" id="transTable">
                    ...
                    <tr>
                        <th width="30%">
                        User Price</h4></th>
                        <td id="transactionId"><input type="text" id="e" name="userprice"  class="form-control" value="<?php echo $row->amount;?>" required></td>
                    </tr> 
  ...
  <script>
    ...
    $('#updatetvplan').submit(function(e){
        e.preventDefault();
        var form = $(this).serialize();
        $.ajax({
            type: "POST",
            url: 'edit_tv_package.php',
            data: form,
            dataType: "json",
...
setTimeout(function(){
document.location.replace('tv-pricing.php');    
},2000);
