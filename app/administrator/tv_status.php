...
$fetchRecord = $conn->query("SELECT * FROM tv_package WHERE network='$network'");
$rowz = $fetchRecord->fetch_assoc();
return json_encode($rowz);
}  
function UpdateSingle($conn,$status,$network){    
$Update_single = $conn->query("UPDATE tv_package SET status='$status' WHERE  network='$network'");
