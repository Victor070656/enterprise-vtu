...
if ($status_fetch !== 'disabled') {
	...
	if (!empty($amount)) {
		...
		$QryInsert = $conn->query("INSERT INTO transactions(network,serviceid,vcode,phone,ref,refer,amount,email,status,token,customer,qty)VALUES('$pnt','$serviceID','$variation_code','$phone','$uid','$token','$amount','$username','$stat','$token','$customerFullName','$iuc')");
		if ($QryInsert) {
			$resp['msg'] = "redirecting please wait";
			$resp['status'] = true;
			$resp['redirect'] = "transaction-view/waec?$uid#transPreview";
