...
$userData = json_decode(fetchUser($conn,$loginId, $loginPhone));
...
function fetchUser($conn,$loginId, $loginPhone){	
$rtmt = $conn-> prepare("SELECT email,firstname,phone FROM users WHERE email =? OR phone=?");
$rtmt -> bind_param('ss',$loginId,$loginPhone);
$rtmt -> execute();
$result = $rtmt -> get_result();
...
//now mail
$DoSendMail = mail($tomail,$subject,$message,$headers);	 
