$('#waecForm').submit(function(e){
                ...
                $.ajax({
                   url: 'formrequest/nbaisRequest.php',
                   type: 'POST',
                   data: data,
                   beforeSend: function(){
 ...
 function showTv(){
    if (document.getElementById('smiledata').checked) {
		...
		var sourceOfPicture = "assets/images/nbais-Result-Checker.jpg";
  ...
  $('#IsData').change(function(){
	 ...
	 var amount = selected.data('amount');
	  ...
	  var phoneN = $('#rctel').val().replace(/[^\d]+/g,'');    
	  $('#tele').val(phoneN);    
	  });
	
	$('#qty1').keyup(function(){
	  var qty1 = $('#qty1').val().replace(/[^\d]+/g,'');    
	  $('#iuc').val(qty1);    
	  });
  ...
  $('#IsVoice').change(function(){
	  ...
	  var phone = $('#rgtel').val().replace(/[^\d]+/g,'');    
	  $('#tele').val(phone);    
	  });
	
	$('#qty2').on('keyup keydown change',function(){
	  var qty2 = $('#qty2').val().replace(/[^\d]+/g,'');    
	  $('#iuc').val(qty2);    
	  });
