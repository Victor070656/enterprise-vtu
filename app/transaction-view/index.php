...
<?php include('../inc/header1.php');?> 
                        ...
                        <div class="row">
							...
							if(isset($_GET['ref'])){
									...
									setTimeout(function(){ window.location.href="dashboard.php" }, 3000);</script>';
										...
										// replace comma
						$str1 = intval($amount);
						$xamount = str_replace( ',', '', intval($str1));
	...
	$airtelvtu = $bil['airtelvtu'];
		...
		$per = $regbil['mtnvtu'];	
			...
			$per = $regbil['airtelvtu'];
				
				}elseif($network === 'etisalat' && $level == $access){
					
				$per = $regbil['9mobilevtu'];
		...
		$affpay = $afi['mtnvtu'];
			...
			$affpay = $afi['airtelvtu'];	
				...
				$payko = strval(($affpay/100) * floatval(intval($xamount)));
				
			$comi = strval(($per/100) * floatval(intval($xamount)));
